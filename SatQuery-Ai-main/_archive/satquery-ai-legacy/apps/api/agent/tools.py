...
@tool(
    name="detect_objects",
    description=(
        "Count salient objects in a single image and return approximate locations. "
        "Class-agnostic (see models/objects/model.py docstring) — does not currently "
        "discriminate by object_class; a real per-class detector needs class-labeled "
        "bounding-box training data this repo doesn't have."
    ),
    input_schema={"image_id": "string"},
    output_schema={"count": "int", "boxes": "list[GeoJSON Feature]", "confidence": "float"},
)
def detect_objects(image_id: str) -> Dict[str, Any]:
    """
    Real model wired in (models/objects/ — density-map regression, not a
    full detector — see that package's docstring for why). Schema note:
    dropped the original object_class input since nothing in this
    implementation can filter by class yet; keeping it would have been an
    unusable parameter kept only for appearance.
    """
    ...
    try:
        result = _object_counter().count(image_row.path)
...
_object_counter_singleton = None


def _object_counter():
    """Lazy singleton so the (small) model loads once, not per-request."""
    global _object_counter_singleton
    if _object_counter_singleton is None:
        from models.objects.infer import ObjectCounter

        _object_counter_singleton = ObjectCounter()
    return _object_counter_singleton
