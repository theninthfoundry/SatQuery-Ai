...
from backend.core.data_class import DataClass
...
class EvidenceContract:
    ...
    def __post_init__(self) -> None:
        ...
        # Auto-derive data_class when not explicitly set, then validate
        # consistency either way — this is what makes Truth Lock 3.0
        # structural rather than a naming convention.
        if self.data_class is None:
            if self.fallback_used:
                self.data_class = DataClass.UNAVAILABLE_CAPABILITY
            elif self.is_real_weights and self.model_ids:
                self.data_class = DataClass.REAL_MODEL
            elif self.model_ids:
                self.data_class = DataClass.DETERMINISTIC_ANALYSIS
            else:
                self.data_class = DataClass.DETERMINISTIC_ANALYSIS

        if self.data_class == DataClass.REAL_MODEL and not self.is_real_weights:
            raise EvidenceContractError(
                "data_class=REAL_MODEL requires is_real_weights=True."
            )
        if self.data_class == DataClass.UNAVAILABLE_CAPABILITY and not self.fallback_used:
            raise EvidenceContractError(
                "data_class=UNAVAILABLE_CAPABILITY requires fallback_used=True."
            )
